...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const isOwner = letter.employeeId._id.toString() === session.user.id;
    ...
    headers.set('Content-Disposition', `attachment; filename="Letter_${letter.employeeId.name?.replace(/\\s+/g, '_') || 'Employee'}.pdf"`);
